...
def adaptive_instance_normalization(content_feat, style_feat):
    ...
    eps = 1e-5
    style_mean = style_feat.mean(dim=-1)
    style_std = (style_feat.var(dim=-1) + eps).sqrt()
    
    content_mean = content_feat.mean(dim=-1)
    content_std = (content_feat.var(dim=-1) + eps).sqrt()

    normalized_feat = (content_feat - content_mean.unsqueeze(-1)) / content_std.unsqueeze(-1)
...
def exact_feature_distribution_matching(content_feat, style_feat):
    ...
    value_content, index_content = torch.sort(content_feat)  # sort conduct a deep copy here.
    value_style, _ = torch.sort(style_feat.unsqueeze(1).repeat(1, C, 1))  # sort conduct a deep copy here.
    inverse_index = index_content.argsort(-1)
    new_content = content_feat + (value_style.gather(-1, inverse_index) - content_feat.detach())
...
class StyleUformerBlock(BasicUformerLayer):
    ...
    def __init__(self, dim, input_dim, output_dim, input_resolution, depth, num_heads, win_size, 
                 ...
                 drop_path=0, norm_layer=nn.LayerNorm, use_checkpoint=False, 
                 token_projection='linear', token_mlp='ffn', shift_flag=True, 
                 modulator=False, cross_modulator=False, style_remover="efdm", style_proj_n_ch=128):
        super().__init__(dim, output_dim, input_resolution, depth, num_heads, win_size, 
        ...
        assert style_remover in self.fn_getter.keys(), "Style remover function should one of this: ['efdm', 'adain', 'hm']"
        
        self.style_remover = self.fn_getter[style_remover]
        self.style_projector = nn.Linear(self.input_dim, style_proj_n_ch)
        
    def extra_repr(self) -> str:
        return super().extra_repr() + f", style_remover={self.style_remover}"
    
    def forward(self, x, feat, mask=None):
        style_proj = self.style_projector(feat)
        return self.style_remover(super().forward(x, mask), style_proj)
    
    def flops(self):
        return super().flops() + self.output_dim * self.input_dim   
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.output_proj = OutputProj(in_channel=2*embed_dim, out_channel=in_chans//3, kernel_size=3, stride=1)
...
class StyleUformer(Uformer):
    def __init__(self, vgg_feats, img_size=256, in_chans=3, dd_in=3, embed_dim=32, style_proj_n_ch=128,
                 depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2], 
                 win_size=8, mlp_ratio=4, qkv_bias=True, qk_scale=None, drop_rate=0, 
                 ...
                 upsample=Upsample, shift_flag=True, modulator=False, cross_modulator=False, style_remover="efdm", **kwargs):
        super().__init__(img_size, in_chans, dd_in, embed_dim, depths, num_heads, win_size, mlp_ratio, qkv_bias,
                         qk_scale, drop_rate, attn_drop_rate, drop_path_rate, norm_layer, patch_norm, use_checkpoint, 
                         token_projection, token_mlp, dowsample, upsample, shift_flag, modulator, cross_modulator,
                         **kwargs)
        # stochastic depth
        enc_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[:self.num_enc_layers]))] 
        ...
        self.destyler = Destyler(in_features=img_size * img_size // 2, num_features=embed_dim)  # from vgg features
        
        
        self.encoderlayer_0 = StyleUformerBlock(dim=embed_dim,
                            input_dim=embed_dim*int(in_chans / 3),
                            output_dim=embed_dim,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            style_proj_n_ch=embed_dim,
                            style_remover=style_remover)
        self.encoderlayer_1 = StyleUformerBlock(dim=embed_dim*2,
                                                input_dim=embed_dim*int(in_chans / 3),
                            output_dim=embed_dim*2,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            style_proj_n_ch=embed_dim*2,
                            style_remover=style_remover)
        self.encoderlayer_2 = StyleUformerBlock(dim=embed_dim*4,
                                                input_dim=embed_dim*int(in_chans / 3),
                            output_dim=embed_dim*4,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            style_proj_n_ch=embed_dim*4,
                            style_remover=style_remover)
        self.encoderlayer_3 = StyleUformerBlock(dim=embed_dim*8,
                                                input_dim=embed_dim*int(in_chans / 3),
                            output_dim=embed_dim*8,
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            style_proj_n_ch=embed_dim*8,
                            style_remover=style_remover)
        self.apply(self._init_weights)
        
    def extra_repr(self) -> str:
        return super().extra_repr() + f", style_proj_n_ch={self.style_proj_n_ch}"
    ...
    def forward(self, x, mask=None):
        ...
        #Encoder
        conv0 = self.encoderlayer_0(y, vgg_feat, mask=mask)
        pool0 = self.dowsample_0(conv0)
        conv1 = self.encoderlayer_1(pool0, vgg_feat, mask=mask)
        pool1 = self.dowsample_1(conv1)
        conv2 = self.encoderlayer_2(pool1, vgg_feat, mask=mask)
        pool2 = self.dowsample_2(conv2)
        conv3 = self.encoderlayer_3(pool2, vgg_feat, mask=mask)
        pool3 = self.dowsample_3(conv3)
...
if __name__ == "__main__":
    ...
    model_restoration = StyleUformer(img_size=input_size, embed_dim=16, depths=depths, style_proj_n_ch=128,
